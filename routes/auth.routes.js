...
router.post("/signup", async (req, res) => {
  ...
  const { git_user, email, password, confirmPassword } = req.body;

  const errors = {};
  // Validacao de nome de usuario: é obrigatório, tem que ser do tipo string e não pode ter mais de 50 caracteres
  if (!git_user || typeof git_user !== "string" || git_user.length > 50) {
    errors.git_user = "Username is required and should be 50 characters max.";
  }
  ...
  if (password !== confirmPassword) {
    errors.confirmPassword = 'Confirmation password and password are different'
  }
  ...
  passport.authenticate("local", async (err, user, info) => {
    try {
      ...
      req.login(user, { session: false }, async (error) => {
        ...
        const body = { _id: user._id, email: user.email, name: user.git_user };
